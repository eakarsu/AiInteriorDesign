...
app.use(createProviderGate(['/api/ai','/api/gap','/api/generated','/api/vendor','/api/contractor','/api/purchase']));
app.get('/api/health',(_req,res)=>res.json({status:'ok',workflow:'approved_interior_design_plan',timestamp:new Date().toISOString()}));
...
app.listen(port,()=>console.log(`Governed interior-design API listening on ${port}`));
