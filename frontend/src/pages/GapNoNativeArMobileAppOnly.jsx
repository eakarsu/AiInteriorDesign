...
// Auto-generated page for: No native AR mobile app (only AR API)

export default function GapNoNativeArMobileAppOnly() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-native-ar-mobile-app-only', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '900px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>No native AR mobile app (only AR API)</h1>
      <p style={{ color: '#666', marginBottom: '1.5rem' }}>No native AR mobile app (only AR API)</p>
      <form onSubmit={handleSubmit} style={{ marginBottom: '1.5rem' }}>
        ...
        >
          {loading ? 'Processing...' : 'Run No native AR mobile app (only AR API)'}
        </button>
      </form>
